...
with gr.Blocks() as chat:
    with gr.Row():
        with gr.Column():
            ...
            with gr.Column(visible=False) as advanced_block:
                ...
                prompt = gr.Textbox(
                    label="prompt", max_lines=100, visible=True)
    ...
    submit_btn.click(LLM.send_query, inputs=[text_model_dropdown, msg_box, detail_output_box, summary_output_box, embed_model_dropdown,
                     topk, score_threshold, temperature, prompt], outputs=[msg_box, detail_output_box, summary_output_box, resp_row, status])
    ...
    yes_btn.click(
        ...
        inputs=[yes_btn, text_model_dropdown, detail_output_box,
                summary_output_box, embed_model_dropdown, topk, score_threshold, temperature],
        outputs=[resp_row, status]
    )
    no_btn.click(
        File_process.save_answer,
        inputs=[no_btn, text_model_dropdown, detail_output_box, summary_output_box, embed_model_dropdown,
                topk, score_threshold, temperature],
        outputs=[resp_row, msg_box, detail_output_box,
                 summary_output_box, status]
    )
    advanced_checkbox.change(advanced_checkbox_change, inputs=[
                             advanced_checkbox], outputs=[advanced_block])
...
with gr.Blocks() as management:
    ...
    with gr.Column(visible=False) as collection_manage:
        with gr.Row():
            rm_file_refresh_btn = gr.Button(
                value="", icon="./icons/refresh.png", interactive=True, elem_classes="btn-refresh")
        with gr.Row():
            with gr.Column():
            ...
            with gr.Column():
                ...
                rm_file_delete_btn = gr.Button(
                    value="移除", interactive=True, visible=False)

    # 管理已儲存的正確答案
    with gr.Column(visible=False) as edit_standard_response:
        with gr.Row():
            edit_csv_dataframe = gr.DataFrame(value=lambda: File_process.dataframe_show(), height=400, column_widths=["30%"],
                                              wrap=True, interactive=False)
        with gr.Row():
            edit_csv_add_row = gr.Button(
                value="新增一列", elem_classes="edit-csv-new-row-btn")
        with gr.Row(visible=False) as edit_section:
            with gr.Row():
                edit_csv_textbox = gr.TextArea(
                    label="編輯框", value="", max_lines=10)
    ...
    function_dropdown.change(function_dropdown_change, inputs=[
                             function_dropdown], outputs=[edit_standard_response, collection_manage])
    # rm_file
    rm_file_refresh_btn.click(
        File_process.filelist_refresh, outputs=[rm_file_dropdown])
    ...
    rm_file_checkbox.change(rm_file_checkbox_change, inputs=[
                            rm_file_checkbox], outputs=[rm_file_delete_btn])
    # edit csv
    edit_csv_dataframe.select(File_process.dataframe_on_select, inputs=[edit_csv_dataframe], outputs=[
                              edit_csv_textbox, edit_csv_row, edit_csv_col, edit_section])
    edit_csv_checkbox.change(edit_csv_checkbox_change, inputs=[
                             edit_csv_checkbox], outputs=[edit_csv_save_btn])
    edit_csv_add_row.click(File_process.dataframe_add_row, inputs=[edit_csv_dataframe],
                           outputs=[edit_csv_dataframe])
    edit_csv_save_btn.click(File_process.dataframe_save_csv,
                            inputs=[edit_csv_dataframe, edit_csv_textbox,
                                    edit_csv_row, edit_csv_col],
